###############################################################################
# A proposal for a more efficient 2-clustering algorithm. #####################
###############################################################################
...
# These are the main variables used in the online updating algorithm.
n1    = (i+bound)
n2    = n - (i+bound)
...
# These variables are more important for the LS itself.
...
mean((beta01+beta11*xx1-yy1)^2)
mean((beta02+beta12*xx2-yy2)^2)
sigmasq1 = (sumysq1 - 2*beta01*sumy1 - 2*beta11*sumxy1 + sumx1*2*beta01*beta11 + beta11^2*sumxsq1 + n1*beta01^2)/n1
sigmasq2 = (sumysq2 - 2*beta02*sumy2 - 2*beta12*sumxy2 + sumx2*2*beta02*beta12 + beta12^2*sumxsq2 + n2*beta02^2)/n2

loglik1 = -1/2*(i+bound)*(1+log(sigmasq1)+log(2*pi))
loglik2 = -1/2*(n-i-bound)*(1+log(sigmasq2)+log(2*pi))
...
coef_df[1,] = c(beta01,beta11,sigmasq1,loglik1,
                beta02,beta12,sigmasq2,loglik2,
                i+bound,score)
...
update_up = function(j){
  ...
  c(beta0 = beta0, beta1 = beta1, sigma = sqrt(sigmasq), loglik = -1/2*n1*(1+log(sigmasq)+log(2*pi)))
}
...
update_down = function(j){
  ...
  c(beta0 = beta0, beta1 = beta1, sigmasq = sqrt(sigmasq), loglik = -1/2*n2*(1+log(sigmasq)+log(2*pi)))
}

for (j in (1:((n-2*bound)-1))) {
  coef_df[j+1,] = c(update_up(i+bound+j),update_down(i+bound+j),j+i+bound,NA)
}
...
ggplot(coef_df,aes(x = Score, y = sqrt(Sigma_upper), colour = Split)) +
